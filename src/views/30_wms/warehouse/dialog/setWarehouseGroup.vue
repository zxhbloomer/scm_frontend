  ...
  >
    ...
    >
      <el-row>
        ...
        >
          <el-transfer
            v-model="settings.transfer.warehouse_groups"
            filterable
            :filter-method="transferFilterMethod"
            filter-placeholder="请输入仓库名称"
            :data="settings.transfer.warehouse_group_all"
            :titles="['未选择仓库分组', '已选择仓库分组']"
            :button-texts="['仓库分组反选', '选择仓库分组']"
          />
        </el-col>
      </el-row>
    </el-form>
...
import { getTransferDataApi, saveTransferDataApi } from '@/api/30_wms/warehouse/warehouse'
...
export default {
  ...
  watch: {
    ...
    'settings.transfer.warehouses': {
      handler (newVal, oldVal) {
        const listA = newVal
        const listB = this.settings.transfer.old_warehouse_groups
  ...
  methods: {
    ...
    // 初始化
    initEdit () {
      this.settings.title = '维护仓库【' + this.data.name + '】分组'
      // 初始化数据
      ...
      getTransferDataApi(this.settings.transfer).then(response => {
        this.settings.transfer.warehouse_group_all = response.data.warehouse_group_all
        this.settings.transfer.warehouse_groups = response.data.warehouse_groups
        this.settings.transfer.old_warehouse_groups = deepcopy(response.data.warehouse_groups)
      }).finally(() => {
    ...
    // 插入逻辑：仓库成员维护，点击确定按钮
    doInsert () {
      saveTransferDataApi(this.settings.transfer).then((_data) => {
