<template>
  <div class="app-container">
        ...
        <template v-slot="scope">
          ...
          <span>
            ...
            <el-link
              ...
              @click="handleEditWarehouseGroupPopup(scope.row.id, scope.row)"
            >
              {{ scope.row.warehouse_group_count }}
            </el-link>
            )
          </span>
        </template>
    ...
    <set-warehouse-group-dialog
      ...
      :serial-data="popSettings.two.props.serialData"
...
<script>
import { getListApi, enableOrDisAbleApi, enabledSelectionApi, disAbledSelectionApi, exportAllApi, exportApi } from '@/api/30_wms/warehouse/warehouse'
import resizeMixin from './mixin/resizeHandlerMixin'
import Pagination from '@/components/Pagination'
import elDragDialog from '@/directive/el-drag-dialog'
import editDialog from '@/views/30_wms/warehouse/dialog/edit'
import deepCopy from 'deep-copy'
import permission from '@/directive/permission/index.js' // 权限判断指令
import SelectDict from '@/components/00_dict/select/SelectDict'
import setWarehouseGroupDialog from '@/views/30_wms/warehouse/dialog/setWarehouseGroup'
import constants_para from '@/common/constants/constants_para'
import FieldHelp from '@/components/30_table/FieldHelp/index.vue'

export default {
  components: { Pagination, editDialog, SelectDict, setWarehouseGroupDialog, FieldHelp },
  ...
  computed: {
    ...
    isRequired () {
      ...
      // shyiyuanth
      if (href.indexOf('shyiyuanth') !== -1 || href.indexOf('172.21.1.249') !== -1) {
        return true
      } else {
        return false
      }
  ...
  methods: {
    ...
    // 点击按钮 更新
    handleUpdate () {
      if (this.dataJson.currentJson.province != null && this.dataJson.currentJson.province !== undefined &&
        this.dataJson.currentJson.city != null && this.dataJson.currentJson.city !== undefined &&
        this.dataJson.currentJson.district != null && this.dataJson.currentJson.district !== undefined) {
        this.dataJson.currentJson.cascader_areas = []
        this.dataJson.currentJson.cascader_areas.push(this.dataJson.currentJson.province * 1)
        this.dataJson.currentJson.cascader_areas.push(this.dataJson.currentJson.city * 1)
        this.dataJson.currentJson.cascader_areas.push(this.dataJson.currentJson.district * 1)
      }
    ...
    handleEditWarehouseGroupPopup (val, row) {
      ...
      this.popSettings.two.props.serialData = ({
        warehouse_id: val
      })
