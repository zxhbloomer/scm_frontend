        ...
        >
          ...
          >
            <span slot-scope="{ option }">
              <span :style="option.enable === false?'color:red':''"> {{ option.label }}</span>
            </span>
          </transfer>
        </el-col>
      </el-row>
...
import { getStaffTransferDataApi } from '@/api/20_master/relation/relation'
...
export default {
  ...
  methods: {
    ...
    initEdit () {
      ...
      getStaffTransferDataApi({ staff_id: this.id }).then(response => {
        this.settings.transfer.warehouse_all = response.data.warehouse_all
        this.settings.transfer.warehouses = this.data.warehouses
        this.settings.transfer.old_warehouses = deepcopy(this.data.warehouses)
      }).finally(() => {
    ...
    // 查看仓库成员
    initView () {
      this.settings.title = '查看员工【' + this.data.name + '】仓库'
      // 初始化数据
      this.settings.transfer = {
        staff_id: this.id,
        // 所有Warehouses
      ...
      this.settings.btnShowStatus.showInsert = true
      getStaffTransferDataApi(this.settings.transfer).then(response => {
        this.settings.transfer.warehouse_all = response.data.warehouse_all
        // 添加新的属性
    ...
    doInsert () {
      const tempDataList = []
      this.settings.transfer.warehouses.forEach((item1, index1) => {
        this.settings.transfer.warehouse_all.forEach((item2, index2) => {
          if (item1 === item2.key) {
            item2.name = item2.label
            tempDataList.push(item2)
          }
        })
      })

      this.$emit('closeMeOk', tempDataList)
      // saveStaffTransferDataApi(this.settings.transfer).then((_data) => {
