  ...
  <div class="qtyLoss">
    ...
    >
      ...
      >
        ...
        >
          <router-link :to="{name:dataJson.constants_program.P_QTY_LOSS,query:{goods_prop:value.goods_prop, fullpath:true}}"><a href="javascript:void(0)">{{ value.qty === null? '' : isZhongLin ? formatNumber3(value.qty) : formatNumberTwo(value.qty,true,2) }}</a></router-link>
        </el-descriptions-item>
      </el-descriptions>
    </el-skeleton>
  </div>
  ...
  methods: {
    getDataList () {
      ...
      getQtyListApi(this.dataJson.searchForm).then(response => {
        ...
        this.dataJson.listData = listData
