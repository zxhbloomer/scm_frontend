...
export default {
  ...
  mounted () {
    // 描绘完成
    this.$nextTick(() => {
      this.$refs.dateChangeLogOrderTabs.$el.children[0].style.margin = 0
      this.$refs.dateChangeLogOrderTabs.$el.children[0].style.display = 'none'
    })
  },
  created () {
    // 获取url
  ...
  methods: {
    ...
    handleView (_data) {
      ...
      this.settings.tabs.activeName = 'edit_datachangelogorder'
