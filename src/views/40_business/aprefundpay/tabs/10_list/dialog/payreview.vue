      ...
      >
        ...
        >
          ...
          <el-descriptions-item>
            ...
            >
              <select-dict
                v-model="dataJson.tempJson.refund_method"
                :para="CONSTANTS.DICT_B_PO_ORDER_PAYMENT_TYPE"
                init-placeholder="请选择付款方式"
              />
            </el-form-item>
          </el-descriptions-item>
          ...
          <el-descriptions-item>
            ...
            <el-form-item
              prop="supplier_enterprise_bank_name"
              label-width="0"
            >
              <el-select
                v-model="dataJson.tempJson.supplier_enterprise_bank_name"
                placeholder="请选择付款方式"
                filterable
                clearable
              >
                <el-option
                  v-for="item in dataJson.bank_collection_data"
                  :key="item.bank_value"
                  :label="item.bank_value"
                  :value="item.bank_value"
                />
              </el-select>
            </el-form-item>
          </el-descriptions-item>
...
import { getBankCollectionDataApi, getDetailApi, paymentReviewApi } from '@/api/40_business/aprefundpay/aprefundpay'
import SelectDict from '@/components/00_dict/select/SelectDict.vue'
import { EventBus } from '@/common/eventbus/eventbus'

export default {
  components: { SelectDict, SimpleUploadMutilFile, PreviewCard },
    ...
    doInsert () {
      ...
      this.$refs['dataSubmitForm'].validate(valid => {
        if (valid) {
          ...
          paymentReviewApi(tempData)
            .then(
              _data => {
                this.closeLoading()
                // 通知兄弟组件，新增数据更新
                EventBus.$emit(this.EMITS.EMIT_MST_B_AP_PAY_UPDATE_OK, _data.data)
                this.$notify({
                  title: '新增成功',
