  ...
  <div>
    ...
    >
      ...
      <el-tab-pane
        name="main"
      >
        <template slot="label">销售订单-列表</template>
        <list_template
          :data="dataJson.data"
          @emitView="handleView"
          @emitNew="handleNew"
          ...
          @emitOutplanNew="handleOutplanNew"
      ...
      <!--出库计划新增-->
      <el-tab-pane
        v-if="dataJson.tab.showOutplanNew"
        name="outplan_new"
        closable
      >
        <template slot="label">{{ dataJson.tab.outplanName }}</template>
        <outplan_new_template
          :so-id="dataJson.so_id"
          @closeMeCancel="handleReturn"
          @closeMeOk="handleCloseMeOk"
        />
      </el-tab-pane>
...
import outplan_new_template from '@/views/40_business/40_out/outplan/component/push/bysoorder/index.vue'
import settlement_new_template from '@/views/40_business/20_so/settlement/component/push/bysoorder/index.vue'
import cargo_right_transfer_new_template from '@/views/40_business/20_so/cargo_right_transfer/component/push/index.vue'

export default {
  components: { list_template, new_template, update_template, detail_template, approve_template, outplan_new_template, settlement_new_template, cargo_right_transfer_new_template },
  ...
  methods: {
    ...
    doCloseTab () {
      ...
      this.dataJson.tab.showOutplanNew = false
     ...
     * @param _data
     */
    handleOutplanNew (_data) {
      this.dataJson.so_id = _data.id
      this.settings.tabs.activeName = 'outplan_new'
      this.dataJson.tab.showNew = false
      this.dataJson.tab.showUpdate = false
      this.dataJson.tab.showView = false
      this.dataJson.tab.showMain = false
      this.dataJson.tab.showApprove = false
      this.dataJson.tab.showOutplanNew = true
      this.dataJson.tab.showSettlementNew = false
      this.dataJson.tab.showCargoRightTransferNew = false
    },
    /**
