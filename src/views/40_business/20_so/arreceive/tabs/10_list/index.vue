<template>
  <div class="app-container">
    <FloatMenu bpm-manage-permission="P_AR_RECEIVE:BPM_MANAGE" />
    <!-- 顶部tabs页签 -->
    <el-tabs
      ref="minusTabs"
    ...
    <div>
      ...
      >
        ...
        <el-form-item>
          <el-input
            v-model.trim="dataJson.searchForm.ar_code"
            clearable
            placeholder="应收账款编号"
            @keyup.enter.native="handleSearch"
          />
    ...
    <div class="div-sum">
      <div class="right">
        ...
        <span class="count-data">{{ dataJson.sumData.sum_receivable_amount_total == null ? 0 : formatCurrency(dataJson.sumData.sum_receivable_amount_total, true) }}</span>
      </div>
    </div>
    ...
    >
      ...
      >
        <template v-slot="scope">
          {{ scope.row.receivable_amount_total == null?'':formatCurrency(scope.row.receivable_amount_total, true) }}
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
      ...
      >
        <template v-slot="scope">
          {{ scope.row.receive_date == null?'':formatDate(scope.row.receive_date) }}
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
...
import {
  getListApi,
  getListSumApi
} from '@/api/40_business/20_so/arreceive/arreceive'
...
import viewDialog from '@/views/40_business/20_so/arreceive/dialog/view/index.vue'
...
import voucherDialog from '@/views/40_business/20_so/arreceive/dialog/voucher/index.vue'
import cancelConfirmDialog from '../../dialog/cancel/index.vue'

export default {
  components: { SelectDicts, SelectCpCustomer, Pagination, viewDialog, SelectSeCustomer, voucherDialog, cancelConfirmDialog },
  ...
  methods: {
    ...
    getDataList () {
      ...
      getListSumApi(this.dataJson.searchForm).then(response => {
        if (response.data !== null) {
          ...
          this.dataJson.sumData.sum_receivable_amount_total = 0
        }
      })
    },
    // 重置查询区域
    ...
    // 收款复核关闭弹窗 收款复核成功
    handleCloseReceiveReviewDialogOk () {
      this.popReceiveReview.dialogVisible = false
      this.getDataList()
    ...
    // 收款复核关闭弹窗 收款复核取消
    handleCloseReceiveReviewDialogCancel () {
      this.popReceiveReview.dialogVisible = false
    },
    // 查看关闭弹窗
