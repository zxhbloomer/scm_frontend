<template>
  <div class="app-container">
          ...
          <el-descriptions-item label="本次申请退款金额">
            ...
            >
              <div style="display: flex; align-items: center;">
                <numeric
                  v-if="dataJson.tempJson.so_contract_code"
                  v-model="dataJson.tempJson.order_amount"
                  :decimal-places="2"
                  :currency-symbol="'¥'"
                  :disabled="isRefundAmountDisabled"
                  style="flex: 1; margin-right: 8px;"
                  @change.native="handleSoOrderCheck(dataJson.tempJson)"
                />
    ...
    <!--(关联单号)收款管理-列表弹窗-->
    <so-order-list-template
      v-if="popSettingsData.arFundDialog.visible"
      :visible="popSettingsData.arFundDialog.visible"
      :data="popSettingsData.arFundDialog.data"
      title="添加关联单-销售预收款"
      @closeMeOk="handleSoorderListCloseOk"
      @closeMeCancel="handleArFountCloseCancel"
    />
...
import { getTypeApi, insertApi, validateApi, getApi } from '@/api/40_business/20_so/arrefund/arrefund'
import { selectListDataApi } from '@/api/10_system/dictdata/dictdata'
import SoOrderListTemplate from '@/views/40_business/20_so/soorder/dialog/listfor/arrefund/index.vue'
...
export default {
  ...
  components: { numeric, SoOrderListTemplate, bank_list_template, BpmDialog, SimpleUploadMutilFile, previewCard },
  mixins: [],
  props: {
    visible: {
      type: Boolean,
    ...
    return {
      ...
      popSettingsData: {
        ...
        sponsorDialog: {
          ...
          visible: false,
          form_data: { },
          serial_type: constants_bpm.BPM_B_AR_REFUND,
          // 点击确定以后返回的值
          selectedDataJson: {
            id: null
          },
  ...
  computed: {
    ...
    isBusinessTypeDisabled () {
      // 当有业务数据时,业务类型控件不可用
      return !!this.dataJson.tempJson.so_contract_code
    },
    // 计算重置清空页面按钮是否禁用
    isResetPageDisabled () {
      // 当有业务数据时,重置按钮可用
  ...
  methods: {
    ...
    initButtonShowStatus () {
      // 初始化按钮状态：默认都隐藏
      this.settings.btArDisabledStatus = this.$options.data.call(this).settings.btArDisabledStatus
      this.settings.btnBankDisabledStatus = this.$options.data.call(this).settings.btnBankDisabledStatus
    },
    // 初始化按钮状态
    initButtonShowStatusReset () {
      // 初始化按钮状态：重置为默认值
      this.settings.btArDisabledStatus.disabledInsert = true
      this.settings.btArDisabledStatus.disabledDelete = true
    ...
    doInsert () {
      ...
      this.$refs['dataSubmitForm'].validate(valid => {
        if (valid) {
          ...
          if (!tempData.so_contract_code) {
            this.showErrorMsg('请选择销售订单')
            return
          }
          ...
          if (tempData.so_contract_code && tempData.order_amount) {
    ...
    // 应收账款获取行索引（单对象模式下不需要索引）
    getArRowIndex (row) {
      return 0 // 单对象模式下索引始终为0
    },
    // 销售订单选择删除
    handleArDelete () {
      if (!this.dataJson.tempJson.so_contract_code) {
      ...
      this.$confirm('是否确认删除该条数据', '确认信息', {
        ...
        // 清空应收账款数据
        this.dataJson.tempJson.ar_code = null
        this.dataJson.tempJson.so_code = null
        this.dataJson.tempJson.project_code = null
        this.dataJson.tempJson.so_contract_id = null
        this.dataJson.tempJson.so_contract_code = null
        this.dataJson.tempJson.so_order_id = null
        this.dataJson.tempJson.so_order_code = null
        this.dataJson.tempJson.so_goods = null
        this.dataJson.tempJson.advance_receive_amount = null
        ...
        // 清空选中行索引
        this.popSettingsData.arFundDialog.rowIndex = null
    ...
    // 添加关联单选择弹窗
    handleArFountInsert () {
      this.popSettingsData.arFundDialog.data = {
        ...
        enterprise_id: this.dataJson.tempJson.seller_id
      }

      this.popSettingsData.arFundDialog.visible = true
    },
    // 重置
    handleArFountRest () {
      // 数据重新初始化 - 直接从初始数据重置
    ...
    // 添加关联单选择完成
    handleSoorderListCloseOk (val) {
      this.popSettingsData.arFundDialog.visible = false
      // 数据整合 - 根据用户需求映射弹窗数据到本页面数据
      this.dataJson.tempJson.so_contract_id = val.so_contract_id
      this.dataJson.tempJson.so_contract_code = val.so_contract_code
      this.dataJson.tempJson.so_order_id = val.so_order_id
      this.dataJson.tempJson.so_order_code = val.so_order_code
      ...
      this.dataJson.tempJson.advance_received_total = val.advance_received_total
      this.dataJson.tempJson.advance_cancelreceive_total = val.advance_cancelreceive_total
      ...
      if (val.detailListData && Array.isArray(val.detailListData) && val.detailListData.length > 0) {
        ...
        this.dataJson.tempJson.so_goods = goodsNames.join(',')
        this.dataJson.tempJson.bankData.so_goods = goodsNames.join(',')
      } else {
        this.dataJson.tempJson.so_goods = ''
        this.dataJson.tempJson.bankData.so_goods = ''
      }

      // 基本信息（保持原有逻辑）
      this.dataJson.tempJson.so_code = val.code
      ...
      this.dataJson.tempJson.enterprise_id = val.seller_id

      // 设置应收账款信息（直接在tempJson下）
      this.dataJson.tempJson.ar_code = val.code
      this.dataJson.tempJson.advance_receive_amount = val.receivable_amount
      ...
      this.querySellerEnterpriseBankAccount(this.dataJson.tempJson.enterprise_id)
    ...
    // 添加关联单选择关闭
    handleArFountCloseCancel () {
      this.popSettingsData.arFundDialog.visible = false
      // 弹窗取消时，重置业务类型选择
      this.dataJson.tempJson.type = null
    },
    // 查询主体企业是否有默认账户
    querySellerEnterpriseBankAccount (enterprise_id) {
      getSellerApi({ enterprise_id: enterprise_id })
        .then(_data => {
          if (_data.data != null) {
    ...
    handleBankInsert () {
      // 根据业务类型设置bank_type数组
      let bankType = []
      if (this.dataJson.tempJson.type === constants_dict.DICT_B_AR_REFUND_TYPE_ONE) {
        // 应收
        bankType = [constants_dict.DICT_M_BANK_TYPE_ONE]
      } else if (this.dataJson.tempJson.type === constants_dict.DICT_B_AR_REFUND_TYPE_TWO) {
        // 预收
        bankType = [constants_dict.DICT_M_BANK_TYPE_TWO]
      } else if (this.dataJson.tempJson.type === constants_dict.DICT_B_AR_REFUND_TYPE_THREE) {
        // 其他收入
    ...
    calculateDetailRefundAmount () {
      // 退款总金额：使用bankData中的order_amount（对应AR模块的退款金额）
    ...
    handleTypeChange (val) {
      // 当选择预收款退款时，弹出选择弹窗
      if (val === constants_dict.DICT_B_AR_TYPE_TWO) {
        this.handleArFountInsert()
      }
    ...
    // 处理退款金额变化
    handleReceivableAmountChange (val) {
      // 触发金额重新计算
      this.calculateDetailRefundAmount()
    },
  ...
  // 消息常量定义
  EMITS: {
    EMIT_MST_B_AR_REFUND_NEW_OK: 'EMIT_MST_B_AR_REFUND_NEW_OK'
  }
}
</script>
