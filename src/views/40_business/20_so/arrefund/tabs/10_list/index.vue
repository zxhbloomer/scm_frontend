<template>
  <div class="app-container">
    <FloatMenu bpm-manage-permission="P_AR_REFUND:BPM_MANAGE" />
    <el-tabs
      ref="minusTabs"
    ...
    >
      ...
      <el-tab-pane name="1">
        <template slot="label">应收退款</template>
      </el-tab-pane>
      <el-tab-pane name="2">
        <template slot="label">预收退款</template>
      </el-tab-pane>
      <el-tab-pane name="3">
      ...
      >
        ...
        <el-form-item>
          <select-dict
            ...
            :para="CONSTANTS.DICT_B_AR_REFUND_PAY_STATUS"
    ...
    <el-button-group>
      ...
      <el-button
        v-permission="'P_AR_REFUND:UPDATE'"
      ...
      <el-button
        v-permission="'P_AR_REFUND:DELETE'"
      ...
      <el-button
        v-permission="'P_AR_REFUND:CANCEL'"
      ...
      <el-button
        v-permission="'P_AR_REFUND:AUDIT'"
      ...
      <el-button
        ...
        v-permission="'P_AR_REFUND:EXPORT'"
        type="primary"
        icon="el-icon-zoom-in"
        :loading="settings.loading"
        @click="handleExport"
      ...
      >导出</el-button>
      <el-button
        v-permission="'P_AR_REFUND:PRINT'"
...
import {
  ...
  getListApi,
  getListSumApi
} from '@/api/40_business/20_so/arrefund/arrefund'
...
export default {
  ...
  beforeDestroy () {
    EventBus.$off(this.EMITS.EMIT_MST_B_AR_REFUND_NEW_OK)
    EventBus.$off(this.EMITS.EMIT_MST_B_AR_REFUND_UPDATE_OK)
  ...
  created () {
    ...
    // 新增提交数据时监听
    EventBus.$on(this.EMITS.EMIT_MST_B_AR_REFUND_NEW_OK, _data => {
      console.log('来自兄弟组件的消息：this.EMITS.EMIT_MST_B_AR_REFUND_NEW_OK', _data)
      // 设置到table中绑定的json数据源
    ...
    // 更新提交数据时监听
    EventBus.$on(this.EMITS.EMIT_MST_B_AR_REFUND_UPDATE_OK, _data => {
      console.log('来自兄弟组件的消息：this.EMITS.EMIT_MST_B_AR_REFUND_UPDATE_OK', _data)
      this.settings.loading = true
      // 查询选中行数据，并更新到选中行的数据
    ...
    // 提交审批流时监听
    EventBus.$on(this.EMITS.EMIT_MST_B_AR_REFUND_BPM_OK, _data => {
      console.log('来自兄弟组件的消息：this.EMITS.EMIT_MST_B_AR_REFUND_BPM_OK', _data)
      this.settings.loading = true
      // 查询选中行数据，并更新到选中行的数据
      getApi({ id: this.dataJson.currentJson.id }).then(response => {
        // 设置到table中绑定的json数据源
  ...
  methods: {
    ...
    handleUpdate () {
      ...
      const operate_tab_data = {
        operate_tab_info: { show: true, name: '修改应收退款' },
        canEdit: true,
    ...
    handleNew () {
      const operate_tab_data = {
        operate_tab_info: { show: true, name: '新增应收退款' },
        canEdit: true,
    ...
    handleView () {
      ...
      const operate_tab_data = {
        operate_tab_info: { show: true, name: '查看应收退款' },
        canEdit: true,
    ...
    handleCurrentChange (row) {
      ...
      if (this.dataJson.currentJson.id !== undefined) {
        ...
        // 待审批和驳回状态，修改，删除按钮高亮
        if (this.dataJson.currentJson.status === constants_dict.DICT_B_AR_REFUND_STATUS_ZERO ||
          this.dataJson.currentJson.status === constants_dict.DICT_B_AR_REFUND_STATUS_THREE) {
        ...
        // 审批
        if (this.dataJson.currentJson.status === constants_dict.DICT_B_AR_REFUND_STATUS_ONE ||
          this.dataJson.currentJson.status === constants_dict.DICT_B_AR_REFUND_STATUS_FOUR
        ) {
        ...
        // 作废
        if (this.dataJson.currentJson.status === constants_dict.DICT_B_AR_REFUND_STATUS_TWO) {
    ...
    handleApprove () {
      ...
      const operate_tab_data = {
        operate_tab_info: { show: true, name: '应收退款审批' },
        canEdit: true,
    ...
    getApprovalStatusText (row) {
      ...
      if (row.status === constants_dict.DICT_B_AR_REFUND_STATUS_ONE || row.status === constants_dict.DICT_B_AR_REFUND_STATUS_FOUR) {
