<template>
  <div class="app-container">
    ...
    <so-order-list-template
      v-if="popSettingsData.arReFundDialog.visible"
      :visible="popSettingsData.arReFundDialog.visible"
      :data="popSettingsData.arReFundDialog.data"
      title="添加关联单-销售预收款"
      @closeMeOk="handleSoorderListCloseOk"
      @closeMeCancel="handleArReFundCloseCancel"
    />
...
import { getTypeApi, insertApi, validateApi } from '@/api/40_business/20_so/arrefund/arrefund'
  ...
  methods: {
    ...
    doInsert () {
      ...
      this.$refs['dataSubmitForm'].validate(valid => {
        if (valid) {
          ...
          // 确保退款附件材料数据被包含
          tempData.refund_doc_att_files = this.dataJson.refund_doc_att

          // 输出附件信息用于调试
          console.log('提交数据中的附件信息:', tempData.refund_doc_att_files)
    ...
    handleArDelete () {
      ...
      this.$confirm('是否确认删除该条数据', '确认信息', {
        ...
        // 清空选中行索引
        this.popSettingsData.arDialog.rowIndex = null
    ...
    // 添加关联单选择弹窗
    handleArReFundInsert () {
      this.popSettingsData.arReFundDialog.data = {
      ...
      this.popSettingsData.arReFundDialog.visible = true
    },
    // 重置
    handleArReFundRest () {
      // 数据重新初始化 - 直接从初始数据重置
    ...
    handleSoorderListCloseOk (val) {
      this.popSettingsData.arReFundDialog.visible = false
    ...
    handleRefundUploadFileSuccess (res) {
      ...
      this.dataJson.tempJson.refund_doc_att_files = this.dataJson.refund_doc_att
    },
    // 退款附件文件移除
