...
import { cancelApi } from '@/api/40_business/20_so/cargo_right_transfer/cargorighttransfer'
  ...
  data () {
    return {
      ...
      popSettingsData: {
        // 审批流程
        sponsorDialog: {
          serial_type: constants_dict.DICT_B_SO_CARGO_RIGHT_TRANSFER_CANCEL,
          // 弹出框显示参数
          visible: false,
          form_data: { },
          // 点击确定以后返回的值
          ...
          cancelApi(tempData)
            .then(
              _data => {
                this.closeLoading()
                // 通知兄弟组件，作废操作完成
                EventBus.$emit(this.EMITS.EMIT_MST_CARGO_RIGHT_TRANSFER_BPM_OK, _data.data)
                this.$notify({
                  title: '新增成功',
