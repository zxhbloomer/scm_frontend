...
export default {
  ...
  methods: {
    ...
    setWatch () {
      ...
      // 监听销售订单列表数据变化，触发金额计算
      this.watch.unwatch_soOrderListData = this.$watch(
        'dataJson.tempJson.soOrderListData',
        (newVal, oldVal) => {
      ...
      // 监听主体企业变化，控制按钮状态
      this.watch.unwatch_seller = this.$watch(
        () => ({
        ...
        (newVal, oldVal) => {
          // 当主体企业有数据时，更新按钮状态
          this.updateButtonStatusBySeller()
        },
        { deep: true, immediate: true }
      )
    },
    unWatch () {
      ...
      if (this.watch.unwatch_soOrderListData) {
        this.watch.unwatch_soOrderListData()
      }
      if (this.watch.unwatch_bankListData) {
        this.watch.unwatch_bankListData()
      }
      if (this.watch.unwatch_seller) {
        this.watch.unwatch_seller()
      }
    },
    // 取消按钮
    handleCancel () {
