        ...
        >
          <template v-slot="scope">
            <div
              ...
              :key="index"
              :class="getClass(index, scope.row.soOrderListData.length)"
            > {{ item.so_goods == null ? '-' : item.so_goods }}
            </div>
          </template>
        </el-table-column>
...
export default {
  components: { Pagination, SelectDict, SelectCpCustomer, SelectSeSeller },
  ...
  methods: {
    ...
    getDataList () {
      ...
      getListByArrefundApi(this.dataJson.searchForm).then(response => {
      ...
      // 查询合计信息
      querySumByArrefund(this.dataJson.searchForm).then(response => {
