...
export default {
  ...
  components: { RadioDict, CustomerDialog, BpmDialog, PreviewCard, SimpleUploadMutilFile, GoodsDialog, SupplierDialog, InputSearch, SelectDict },
  mixins: [],
  props: {
    data: {
      type: Object,
          ...
          insertApi(tempData)
            .then(
              _data => {
                ...
                // 通知兄弟组件，新增数据更新
                setTimeout(() => {
                  EventBus.$emit(this.EMITS.EMIT_MST_SOCONTRACT_NEW_OK, _data.data)
                }, 1000)
                this.$notify({
                  title: '新增成功',
