<template>
  <div class="app-container">
      ...
      >
        ...
        <el-form-item>
          <select-dicts
            ...
            :para="CONSTANTS.DICT_B_IN_V2_STATUS"
    ...
    />
    <!--    <instance-preview v-if="popSettings.process.visible" instance-id="1" />-->
    <el-drawer
      :visible.sync="popSettings.process.visible"
      :size="isMobile ? '100%' : '500px'"
      direction="rtl"
      title="审批详情"
    >
      <instance-preview instance-id="1" />
    </el-drawer>
...
import { getTodoCountApi, getListSumApi, exportAllBInApi, exportBInApi, importBInApi, getListApi, submitApi, returnApi, cancelApi, syncApi, syncAllApi, getCountApi, cancelReturnApi } from '@/api/40_business/inv2/inv2'
...
import cancelConfirmDialog from '@/views/40_business/inv2/tabs/10_list/dialog/cancelconfirm.vue'
...
import mixin from '@/views/40_business/inv2/tabs/10_list/mixin'
import constants_dict from '@/common/constants/constants_dict'

export default {
  components: { SelectSupplier, Pagination, SelectDicts, SelectDict, SelectConsignor, SelectWarehouse, SimpleUpload, cancelConfirmDialog, InstancePreview },
  ...
  methods: {
    ...
    handleUpdate () {
      ...
      if (_data.status === constants_dict.DICT_B_IN_STATUS_V2_TWO) {
        this.showErrorMsg('已审批的数据不允许操作')
        return
      }
      if (_data.status === constants_dict.DICT_B_IN_PLAN_STATUS_V2_FIVE) {
    ...
    handleCurrentChange (row) {
      ...
      if (this.dataJson.currentJson.id !== undefined) {
        ...
        // 审批中和驳回状态，修改按钮高亮
        if (this.dataJson.currentJson.status === constants_dict.DICT_B_IN_V2_STATUS_ZERO ||
          this.dataJson.currentJson.status === constants_dict.DICT_B_IN_V2_STATUS_THREE) {
        ...
        if (this.dataJson.currentJson.status === constants_dict.DICT_B_IN_V2_STATUS_TWO && this.dataJson.currentJson.monitor_in_id == null) {
        ...
        // 审批
        if (this.dataJson.currentJson.status === constants_dict.DICT_B_IN_V2_STATUS_ONE ||
          this.dataJson.currentJson.status === constants_dict.DICT_B_IN_V2_STATUS_FOUR
        ) {
    ...
    handleTabsClick (tab, event) {
      ...
      switch (tab.index) {
        case '1': // 待审批
          this.dataJson.searchForm.status = constants_type.DICT_B_IN_STATUS_V2_ZERO
          break
        case '2': // 审批中
          this.dataJson.searchForm.status = constants_type.DICT_B_IN_STATUS_V2_ONE
          break
        case '3': // 执行中
          this.dataJson.searchForm.status = constants_type.DICT_B_IN_STATUS_V2_TWO
          break
        case '4': // 已完成
          this.dataJson.searchForm.status = constants_type.DICT_B_IN_STATUS_V2_SIX
          break
        case '5': // 驳回
          this.dataJson.searchForm.status = constants_type.DICT_B_IN_STATUS_V2_THREE
          break
        case '6': // 作废审批中
          this.dataJson.searchForm.status = constants_type.DICT_B_IN_STATUS_V2_FOUR
          break
        case '7': // 已作废
          this.dataJson.searchForm.status = constants_type.DICT_B_IN_STATUS_V2_FIVE
          break
        default:
