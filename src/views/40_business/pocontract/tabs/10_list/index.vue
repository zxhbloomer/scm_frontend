...
} from '@/api/40_business/pocontract/pocontract'
...
import print_template from '@/views/40_business/pocontract/tabs/60_print/index.vue'
import push_order_template from '@/views/40_business/poorder/dialog/push/new/index.vue'
...
export default {
  ...
  methods: {
    ...
    handleDel () {
      const _data = deepCopy(this.dataJson.currentJson)
      // 状态为待审批才可以删除
      if (_data.status.toString() !== constants_type.DICT_B_PO_CONTRACT_STATUS_ZERO) {
        this.showErrorMsg('采购合同状态异常')
        return
      }
