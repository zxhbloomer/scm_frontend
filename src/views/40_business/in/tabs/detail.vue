    ...
    <div class="edit-container">
      ...
      >
        <el-descriptions
          ...
          style="padding-right: 10px;padding-left: 10px;"
        >
          <el-descriptions-item label="入库仓库/库区/库位">{{ dataJson.tempJson.warehouse_name }}/{{ dataJson.tempJson.location_name }}/{{ dataJson.tempJson.bin_name }}</el-descriptions-item>
          <el-descriptions-item label="车牌号">{{ dataJson.tempJson.vehicle_no == null? '':dataJson.tempJson.vehicle_no }}</el-descriptions-item>
          <el-descriptions-item>
            <div slot="label">
              实收车数
            </div>
          ...
          <el-descriptions-item label="原发数量">{{ formatNumber(dataJson.tempJson.primary_quantity, true, 4) }}</el-descriptions-item>
          <el-descriptions-item
            label="毛重"
          >
            {{ formatNumber(dataJson.tempJson.gross_weight === undefined ? 0 : dataJson.tempJson.gross_weight, true, 4) }}
            <span class="floatRight">{{ dataJson.tempJson.unit_name }}</span>
          </el-descriptions-item>
          <el-descriptions-item
            label="皮重"
          >
            {{ formatNumber(dataJson.tempJson.tare_weight === undefined ? 0 : dataJson.tempJson.tare_weight, true, 4) }}
            <span class="floatRight">{{ dataJson.tempJson.unit_name }}</span>
          </el-descriptions-item>
          <el-descriptions-item>
            <div slot="label">
              入库数量
              <span v-if="settings.weight_control_config">（净重）</span>
            </div>
            <el-form-item
              prop="actual_count"
              label-width="0"
        ...
        >
          ...
          <el-descriptions-item v-if="!settings.weight_control_config && dataJson.tempJson.status !=='4'" />
...
export default {
  ...
  methods: {
    ...
    countChange (val) {
      ...
      this.dataJson.tempJson.total_price = this.dataJson.tempJson.actual_count * this.dataJson.tempJson.price
    },
    // 价格改变
    priceChange (val) {
      this.dataJson.tempJson.total_price = val * this.dataJson.tempJson.actual_count
    },
    getData () {
      // 查询逻辑
      this.settings.loading = true
