  ...
  <div class="app-container">
    <div class="edit-container">
      ...
      >
        ...
        <el-button-group>
          <el-button
            :disabled="settings.btApDisabledStatus.disabledInsert"
            type="primary"
          ...
          >添加</el-button>
          <el-button
            :disabled="settings.btApDisabledStatus.disabledDelete"
            type="primary"
        ...
        <el-table
          ...
          style="width: 100%"
          @row-click="handleApRowClick"
          @current-change="handleApCurrentChange"
        >
          <el-table-column
            type="index"
            width="45"
...
import { getTypeApi, insertApi, validateApi } from '@/api/40_business/aprefund/aprefund'
...
export default {
  ...
  components: { numeric, ApListTemplate, bank_list_template, BpmDialog },
  mixins: [],
  props: {
    visible: {
      type: Boolean,
    ...
    return {
      ...
      settings: {
        ...
        rules: {
          ...
          refunding_amount: [
            { required: true, message: '请添加申请退款金额', trigger: 'change' }
          ],
          refund_amount: [
  ...
  methods: {
    ...
    // 应付账款选中
    handleApRowClick (row) {
      this.popSettingsData.apDialog.rowIndex = this.getApRowIndex(row)
    },
    // 应付账款获取行索引
    getApRowIndex (row) {
    ...
    // 应付账款按钮
    handleApCurrentChange (row) {
      this.dataJson.currentJson = Object.assign({}, row) // copy obj
      this.dataJson.currentJson.index = this.getApRowIndex(row)
      this.settings.btApDisabledStatus.disabledDelete = false
    },
    // 应付账款选择完成
    handleApCloseOk (val) {
      ...
      tempData.refunded_amount = val.refunded_amount
      tempData.refunding_amount = val.refunding_amount
    ...
    // 采购订单选择删除
    handleApDelete () {
      if (this.popSettingsData.apDialog.rowIndex === null) {
      ...
      this.$confirm('是否确认删除该条数据', '确认信息', {
        ...
        this.settings.btApDisabledStatus.disabledInsert = false
      }).catch(() => {
      })
    },
    // 添加关联单选择弹窗
