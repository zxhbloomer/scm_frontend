      ...
      >
          ...
          >
            ...
            >
              ...
              <el-descriptions-item label="仓库">
                {{ dataJson.tempJson.wc_warehouse_name }}
              </el-descriptions-item>
        ...
        >
          ...
          <el-descriptions-item label="产成品，副产品产量公式">
            产成品、副产品产量=产成品产量 ÷ 产成品配比 : {{ formatNumber(dataJson.sumData.prod_all_num, 4) }}<br class="fontBr">
            产成品产量=产成品、副产品产量 * 产成品配比 : {{ formatNumber(dataJson.sumData.prod_num, 4) }}<br class="fontBr">
            ...
            <div class="grid-content bg-purple-light">
              ...
              <span class="count_1">
                ...
                <font color="red">
                  ...
                  {{ formatNumber(dataJson.sumData.material_actual , 4) }}
                </font>
              </span>
            </div>
          </el-col>
...
import { insertApi, updateApi, getDetailApi, checkApi, checkQtyApi, getInventoryApi } from '@/api/40_business/wo/wo'
...
export default {
  ...
  methods: {
    ...
    doInsert () {
      this.$refs['dataSubmitForm'].validate(valid => {
        if (valid) {
          ...
          // 计算产成品, 副产品配比
          const productRouter = tempData.product_list.map(item => item.wo_router).reduce((pre, cur) => pre + cur)
          if (productRouter !== 100) {
            this.showErrorMsg('产成品,副产品明细的配比之和应为100%')
            return
          }
          const materialRouter = tempData.material_list.map(item => item.wo_router).reduce((pre, cur) => pre + cur)
    ...
    // 产成品, 副产品选择规格
    handleProductSpecSelect (id, data) {
      // this.dataJson.tempJson.product_list.splice(index, 1, product)

      // if (this.PARAMETERS.STATUS_INSERT === this.editStatus || (this.PARAMETERS.STATUS_UPDATE === this.editStatus && this.settings.isPicthOnProduct)) {
      //   // 获取当前仓库的下标
      //   const index = this.dataJson.productCurrentJson.index
      //   const product = this.dataJson.tempJson.product_list[index]
      //   product.sku_id = val
      //   this.dataJson.tempJson.product_list.splice(index, 1, product)
      // }
    },
    ...
    handleProductUnitChange (val, row) {
      ...
      //     // item.unit_id = deepCopy(val.src_unit_id)
      //     // item.unit_name = deepCopy(val.src_unit)
      //     this.$set(item, 'unit_id', val.src_unit_id)
      //     this.$set(item, 'unit_name', val.src_unit)
      //   }
      // })
    ...
    handleMaterialUnitChange (val, row) {
      ...
      //     this.$set(item, 'unit_id', val.src_unit_id)
      //     this.$set(item, 'unit_name', val.src_unit)
      //     // this.$set(item, 'unit_settings', true)
      //   }
      // })
    },
    // 根据原材料计算副产品数量
    handleProductNum (val, row) {
      // if (row !== null) {
      //   const wo_router = row.wo_router
      //   const wo_qty = row.wo_qty
      //   const qty = wo_qty / (wo_router / 100)
      //   // 计算副产品数量
      //   for (var i = 0; i < this.dataJson.tempJson.product_list.length; i++) {
      //     const product = this.dataJson.tempJson.product_list[i]
      //     if (row.type === constants_type.DICT_B_ROUTER_PRODUCT_TYPE_C && product.type === constants_type.DICT_B_ROUTER_PRODUCT_TYPE_F) {
      //       product.wo_qty = qty * product.wo_router / 100
      //       this.$set(this.dataJson.tempJson.product_list[i], 'goods_prop', product.goods_prop + ' ') // 强制更新下属性, 不然那列表不刷新
      //     }
      //   }
      //   // 计算原材料数量
      //   for (var i1 = 0; i1 < this.dataJson.tempJson.material_list.length; i1++) {
      //     const material = this.dataJson.tempJson.material_list[i1]
      //     if (row.type === constants_type.DICT_B_ROUTER_PRODUCT_TYPE_C) {
      //       material.wo_qty = qty * material.wo_router / 100
      //       this.$set(this.dataJson.tempJson.material_list[i1], 'goods_prop', material.goods_prop + ' ') // 强制更新下属性, 不然那列表不刷新
      //     }
      //   }
      //   // this.material_num = this.dataJson.tempJson.material_list.map(item => item.wo_qty).reduce((pre, cur) => (pre + cur))
      // }
      this.handleCheck()
    },
    // 清空配方表
   ...
   */
    ...
    sumPorductNum () {
      ...
      // 产成品, 副产品产量
      const qty = product.wo_qty / (product.wo_router / 100)
      this.dataJson.sumData.prod_all_num = qty
      // 获得副产品的比例
      ...
      if (productRouterList.length) {
        productRouter = productRouterList.map(item => item.wo_router).reduce((pre, cur) => pre + cur)
      }
