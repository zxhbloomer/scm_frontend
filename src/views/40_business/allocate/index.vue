  ...
  <div>
    ...
    >
      <el-tab-pane name="main">
        <template slot="label">调拨单</template>
        <allocate-template
...
import allocateTemplate from './tabs/list'
import editTemplate from './tabs/edit'
import resizeMixin from '@/mixin/resizeHandlerMixin'

export default {
  components: { allocateTemplate, editTemplate },
  ...
  mounted () {
    // 描绘完成
    this.$nextTick(() => {
      this.$refs.allocateTabs.$el.children[0].style.margin = 0
      this.$refs.allocateTabs.$el.children[0].style.display = 'none'
    })
  },
  created () {
  },
  methods: {
    ...
    handleInsert (_data) {
      ...
      this.settings.tabs.activeName = 'edit_allocate'
