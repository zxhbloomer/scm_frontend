  ...
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        <!--        </template>-->
        <template v-slot="scope">
          <a href="javascript:void(0)" @click="toMonitorDetail(scope.row.type,scope.row.type_name,scope.row.id,scope.row.code)">{{ scope.row.monitor_count == null?0:scope.row.monitor_count }}</a>
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      // 只查询 1物流订单, 2物流调度 4:直采入库 5:直销出库
      this.dataJson.searchForm.type_list = [constants_dict.DICT_B_SCHEDULE_STATUS_ONE, constants_dict.DICT_B_SCHEDULE_TYPE_TWO, constants_dict.DICT_B_SCHEDULE_TYPE_FOUR, constants_dict.DICT_B_SCHEDULE_TYPE_FIVE]
    ...
    // 点击派车数
    toMonitorDetail (type, type_name, id, code) {
      const _this = this
      isBackupApi({ schedule_id: id }).then(response => {
        ...
        if (response.data.is_backup === '1') {
          ...
          _this.$confirm(type_name + '【' + code + '】下已有部分数据已备份，请选择需要查看的数据', '提示', {
            ...
            // 历史记录
            this.monitorbackupRouter(type, code)
          })
    ...
    monitorRouter (type, code) {
      const _this = this
      // 通知路由，打开直采直销监管任务页面
      if (type === constants_dict.DICT_B_SCHEDULE_TYPE_FOUR || type === constants_dict.DICT_B_SCHEDULE_TYPE_FIVE) {
        _this.$router.push({ // 开直采直销监管任务页面
          path: '/monitordirect/index', query: { schedule_code: code, fullpath: true }
        })
      } else {
        _this.$router.push({ // 打开监管任务页面
          path: '/monitor/index', query: { schedule_code: code, fullpath: true }
        })
      }
    },
    monitorbackupRouter (type, code) {
      const _this = this
      if (type === constants_dict.DICT_B_SCHEDULE_TYPE_FOUR || type === constants_dict.DICT_B_SCHEDULE_TYPE_FIVE) {
        _this.$router.push({
          path: '/monitorbackupdirectv2/index', query: { schedule_code: code, fullpath: true }
        })
      } else {
        _this.$router.push({
