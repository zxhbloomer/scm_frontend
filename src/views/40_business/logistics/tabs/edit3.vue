      ...
      >
        ...
        >
          ...
          <el-descriptions-item label="单据类型">
            {{ dataJson.tempJson.type_name == null?'直采入库': dataJson.tempJson.type_name }}
          </el-descriptions-item>
        ...
        >
          ...
          <el-descriptions-item label="采购企业">
...
import { getApi, getScheduleQtyApi, insert4Api, update4Api } from '@/api/40_business/logistics/logistics'
...
import inWarehouseDialog from '@/views/30_wms/warehouse/dialog/10_list/index.vue'
import outWarehouseDialog from '@/views/30_wms/warehouse/dialog/10_list/index.vue'
import outplanDialog from '@/views/40_business/40_out/outplan/dialog/select_goods'
...
export default {
    ...
    return {
      ...
      dataJson: {
        ...
        searchScheduleForm: {
          ...
          sku_id: null,
          type: constants_dict.DICT_B_SCHEDULE_TYPE_FOUR
        },
        // 可用库存量
        schedule_qty: 0,
        // 单条数据 json的，初始化原始数据
  ...
  methods: {
    ...
    handleOrderDialog () {
      ...
      this.popSettingsData.searchDialogDataEight.data = { sales_contract_code: this.dataJson.tempJson.sales_contract_code, sales_serial_type: constants_dict.DICT_B_ORDER_IN, type: 'b_schedule_order' }
    ...
    handleInRuleChange (val) {
      ...
      if (val === this.CONSTANTS.DICT_B_LOGISTICS_IN_RULE_0) {
        ...
        this.dataJson.tempJson.in_plan_detail_id = null
        this.dataJson.tempJson.in_warehouse_id = null
    ...
    getData () {
      ...
      getApi(this.data).then(response => {
        ...
        // 直采入库 运输数量取值入库数量
        tempData.actual_count = data.in_schedule_qty
    ...
    // 入库计划
    handleInPlanDialog () {
      this.popSettingsData.searchDialogDataTwelve.data = { contract_no: this.dataJson.tempJson.contract_no, type: 'b_schedule', filter_type: [this.CONSTANTS.DICT_B_IN_PLAN_TYPE_TIH, this.CONSTANTS.DICT_B_IN_PLAN_TYPE_JG_TH] }
      if (this.dataJson.tempJson.contract_no === null || this.dataJson.tempJson.contract_no === '') {
        this.showErrorMsg('请先选择合同编号')
        return
      }
    ...
    handleInPlanCloseOk (val) {
      ...
      // 自动生成出库计划，收货区域手动选择入库计划
      if (this.dataJson.tempJson.out_rule === '0') {
        var tempData = {}
        tempData.sku_code = val.sku_code
        tempData.goods_code = val.goods_code
        tempData.sku_id = val.sku_id
