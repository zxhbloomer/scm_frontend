        ...
        <el-row>
          <el-col :span="12">
            ...
            >
              ...
              /> -->
              <numeric
                v-model="dataJson.tempJson.contract_num"
                :minimum-value="0"
                :decimal-places="0"
                currency-symbol=""
                :disabled="isViewModel"
...
import { updateApi, insertApi } from '@/api/40_business/ownerchangeorder/ownerchangeorder'
import supplierDialog from '@/views/30_wms/customer/dialog/dialog'
import numeric from '@/components/40_input/numeric'

export default {
  components: { SelectDict, supplierDialog, numeric },
    ...
    return {
      ...
      settings: {
        ...
        rules: {
          ...
          contract_num: [
            { validator: this.checkContractNum, trigger: 'change' }
          ]
        },
        // 错误数目
        badge: {
  ...
  methods: {
    ...
    // 校验合同量需要大于0
    checkContractNum (rule, value, callback) {
      if (value <= 0 || value === undefined || value === null) {
        callback(new Error('合同量必须大于0'))
      } else {
        callback()
      }
    }
