      ...
      >
        ...
        >
          ...
          <el-descriptions-item label="超发比例(%)">
            {{ dataJson.planData.over_delivery_rate == null ? '-' : formatNumber(dataJson.planData.over_delivery_rate * 100, true, 2) }}
          </el-descriptions-item>
          <el-descriptions-item label="备注" span="3">
          ...
          <el-descriptions-item label="计划出库数量（吨）">
            {{ formatNumber(dataJson.planData.plan_qty, true, 4) || '-' }}
          </el-descriptions-item>
          <el-descriptions-item label="计划出库单价">
  ...
  methods: {
    ...
    handleDialogReturnData (data) {
      ...
      this.dataJson.planData = {
        ...
        over_delivery_rate: planData.over_delivery_rate || null,
      ...
      this.dataJson.tempJson.customer_id = selectedRow.customer_id
      this.dataJson.tempJson.customer_name = selectedRow.customer_name
      this.dataJson.tempJson.customer_code = selectedRow.customer_code || ''
      ...
      // 设置出库类型（从出库计划数据中获取）
      this.dataJson.tempJson.type = planData.type || constants_dict.DICT_B_OUT_TYPE_XS
      this.dataJson.tempJson.type_name = planData.type_name || '销售出库'

      // 设置默认出库时间为当前时间
      // this.dataJson.tempJson.outbound_time = new Date().toISOString().slice(0, 19).replace('T', ' ')

      console.log('=== 弹窗数据填充完成 ===')
      console.log('当前出库单数据:', this.dataJson.tempJson)
      console.log('出库计划数据:', this.dataJson.planData)
    ...
    handleDirectData (data) {
      ...
      this.dataJson.planData = {
        ...
        over_delivery_rate: data.over_delivery_rate || null,
      ...
      this.dataJson.tempJson.customer_id = data.customer_id
      this.dataJson.tempJson.customer_name = data.customer_name
      this.dataJson.tempJson.customer_code = data.customer_code || ''
      ...
      this.dataJson.tempJson.type_name = '销售出库'
    ...
    initComponent () {
      ...
      this.dataJson.tempJson.outbound_time = null
          ...
          insertApi(tempData)
            .then(
              _data => {
                ...
                // 通知兄弟组件，新增数据更新
                setTimeout(() => {
                  EventBus.$emit('EMIT_MST_B_OUT_NEW_OK', _data.data)
                }, 1000)
                this.$notify({
                  title: '出库单提交成功',
                  message: _data.data.message || '出库单已提交审批并保存',
                  type: 'success',
                  duration: this.settings.duration
                })
              },
    ...
    buildSubmissionData () {
      ...
      const submissionData = {
        ...
        // 出库计划信息
        plan_id: this.dataJson.planData.plan_id, // 出库计划ID
        plan_detail_id: this.dataJson.planData.plan_detail_id, // 出库计划明细ID
        plan_code: this.dataJson.planData.plan_code || '', // 出库计划编号
        ...
        two_file: this.dataJson.two_file || [], // 出库附件明细
