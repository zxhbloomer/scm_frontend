...
import { updateApi, getApi, validateApi } from '@/api/40_business/40_out/out/out'
...
export default {
    ...
    return {
      ...
      settings: {
        ...
        inputDisabledStatus: {
          ...
          disabledOutboundTime: false, // 出库时间
          disabledQty: false, // 出库数量
          disabledPrice: false // 出库单价
        },
        rules: {
          wlb_data: [
      ...
      popSettingsData: {
        ...
        sponsorDialog: {
          visible: false,
          serial_type: constants_bpm.BPM_B_OUT, // 出库单更新审批流程
          form_data: {},
  ...
  methods: {
    ...
    handleTwoFileUploadSuccess (res) {
      ...
      console.log('出库附件明细上传成功', res.response)
    },
    ...
    startProcess () {
      ...
      this.$refs['dataSubmitForm'].validate(valid => {
        if (valid) {
          ...
          if (!tempData.outbound_time) {
            this.closeLoading()
            this.showErrorMsg('请选择出库时间')
            return
          }
          ...
          updateApi(tempData)
            .then(
              _data => {
                ...
                // 通知兄弟组件，数据更新
                setTimeout(() => {
                  EventBus.$emit(this.EMITS.EMIT_MST_B_OUT_UPDATE_OK, _data.data)
                }, 1000)
                this.$notify({
                  title: '更新成功',
