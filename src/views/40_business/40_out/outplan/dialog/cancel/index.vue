...
import { cancelApi } from '@/api/40_business/40_out/outplan/outplan'
  ...
  data () {
    return {
      ...
      popSettingsData: {
        // 审批流程
        sponsorDialog: {
          serial_type: constants_dict.DICT_B_OUT_PLAN_CANCEL,
          // 弹出框显示参数
          visible: false,
          form_data: {},
          // 点击确定以后返回的值
          ...
          cancelApi(tempData)
            .then(
              _data => {
                // 通知兄弟组件，数据更新
                setTimeout(() => {
                  EventBus.$emit(this.EMITS.EMIT_MST_OUTPLAN_UPDATE_OK, _data.data)
                }, 1000)
