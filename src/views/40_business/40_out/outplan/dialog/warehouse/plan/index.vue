  ...
  data () {
    return {
      ...
      settings: {
        ...
        rules: {
          qty: [
            { required: true, message: '请输入本次计划出库数量', trigger: 'blur' },
      ...
      dataJson: {
        ...
        tempJson: {
          ...
          order_qty: 0, // 订单数量
          order_price: 0, // 订单单价
          processed_qty: 0, // 已出库数量
          unprocessed_qty: 0, // 待出库数量
          price: null // 出库单价
        },
        tempJsonOriginal: {}
      }
  ...
  methods: {
    ...
    initDataJson () {
      ...
      if (this.data) {
        ...
        // 出库单价默认等于订单单价，如果已经有设置过的价格则使用设置的价格
    ...
    qty_validator (rule, value, callback) {
      if (value <= 0) {
        callback(new Error('本次计划出库数量必须大于0'))
      } else {
        callback()
      }
    },
