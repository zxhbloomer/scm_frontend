    ...
    <div class="edit-container">
      ...
      >
        ...
        >
          ...
          <el-descriptions-item span="2" label="出库类型">
            <span>销售出库</span>
          </el-descriptions-item>
...
import { getApi } from '@/api/40_business/20_so/soorder/soorder.js'
import WarehouseSetDialog from '@/views/40_business/40_out/outplan/dialog/warehouse/plan/index.vue'
  ...
  methods: {
    ...
    getData () {
      ...
      getApi({ id: this.soId }).then(response => {
        ...
        // 调用初始化计划数据API
        initPlanDataApi({ id: this.soId, order_id: this.soId }).then(response => {
          // 将计划数据赋值给detailListData
          this.dataJson.tempJson.detailListData = deepCopy(response.data)
        }).catch(error => {
          console.error('获取出库计划数据失败:', error)
