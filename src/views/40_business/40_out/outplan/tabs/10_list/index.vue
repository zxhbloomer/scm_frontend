<template>
  <div class="app-container">
    <FloatMenu bpm-manage-permission="P_OUT_PLAN:BPM_MANAGE" />
    <!-- 状态标签页 -->
    <el-tabs
      ref="minusTabs"
    ...
    >
      <div class="right">
        <span class="count-title">总出库数量：</span><span class="count-data">{{ dataJson.sumData.qty_sum == null ? 0 : formatNumber(dataJson.sumData.qty_sum,true,4) }}</span>
      </div>
    </div>
    <!-- 数据表格 -->
    <el-table
    ...
    >
      ...
      />
      <!-- 出库计划主表 -->
      <el-table-column label="出库计划主表" align="center">
        <el-table-column
          sortable="custom"
...
export default {
  ...
  data () {
    return {
      ...
      showSelectGoodsDialog: false, // 显示出库货物选择弹窗
      selectedOutPlanId: null // 选中的出库计划ID
    }
  },
  computed: {
  },
  ...
  created () {
    ...
    // 更新提交数据时监听
    EventBus.$on(this.EMITS.EMIT_MST_OUTPLAN_UPDATE_OK, _data => {
      console.log('来自兄弟组件的消息：this.EMITS.EMIT_MST_OUTPLAN_UPDATE_OK', _data)
      this.settings.loading = true
      // 查询选中行数据，并更新到选中行的数据
    ...
    // 提交审批流时监听
    EventBus.$on(this.EMITS.EMIT_MST_OUTPLAN_BPM_OK, _data => {
      console.log('来自兄弟组件的消息：this.EMITS.EMIT_MST_OUTPLAN_BPM_OK', _data)
      this.settings.loading = true
      // 查询选中行数据，并更新到选中行的数据
      getApi({ id: this.dataJson.currentJson.id }).then(response => {
        // EventBus.$off(this.EMITS.EMIT_MST_ENTERPRISE_NEW_OK)
  ...
  beforeDestroy () {
    // 清理EventBus监听器
    EventBus.$off(this.EMITS.EMIT_MST_OUTPLAN_NEW_OK)
    EventBus.$off(this.EMITS.EMIT_MST_OUTPLAN_UPDATE_OK)
    EventBus.$off(this.EMITS.EMIT_MST_OUTPLAN_BPM_OK)
  },
  methods: {
    ...
    handleUpdate () {
      ...
      const operate_tab_data = {
        operate_tab_info: { show: true, name: '修改出库计划' },
        canEdit: true,
    ...
    handleApprove () {
      ...
      const operate_tab_data = {
        operate_tab_info: { show: true, name: '出库计划审批' },
        canEdit: true,
    ...
    handleOutbound () {
      ...
      // 打开出库货物明细选择弹窗
      this.selectedOutPlanId = _data.id
    ...
    handleSelectGoodsNext (data) {
      ...
      const operate_tab_data = {
        ...
        operate_tab_info: {
          show: true,
          name: '出库操作-' + data.planData.code
        },
        editStatus: 'new'
      }

      this.$emit('emitOutbound', operate_tab_data)
    },
    ...
    handleFinish () {
      ...
      this.$confirm('完成后不可再进行任何操作，确认要完成该出库计划吗？', '确认信息', {
      }).then(() => {
        this.handleFinishOk()
      }).catch(action => {
        // 右上角X
