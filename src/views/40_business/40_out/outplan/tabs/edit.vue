<template>
  <div class="app-container">
    ...
    <bpm-dialog
      ...
      :serial-type="constants_dict.DICT_BPM_TODO_SERIAL_TYPE_OUT_PLAN"
      @closeSponsorCancel="closeSponsorCancel"
      @closeSponsorOk="closeSponsorOk"
    />
  </div>
</template>
...
import { updateApi, getApi, getOverReleaseConfigApi, newInsertApi } from '@/api/40_business/outplan/outplan'
// import { getListApi } from '@/api/10_system/appcode/config'
import consignorDialog from '@/views/30_wms/customer/dialog/dialog.vue'
import ownerDialog from '@/views/30_wms/owner/dialog/dialog.vue'
import orderDialog from '@/views/40_business/order/dialog/dialog.vue'
import inventoryDialog from '@/views/40_business/40_out/outplan/tabs/dialog/inventoryEdit.vue'
...
export default {
  components: { BpmDialog, InputSearch, SelectDict, consignorDialog, ownerDialog, orderDialog, inventoryDialog },
  ...
  methods: {
    ...
    startProcess () {
      ...
      // 查询是否存在审批流数据
      getFlowProcessApi({ 'serial_type': constants_dict.DICT_BPM_TODO_SERIAL_TYPE_OUT_PLAN })
        .then((rsp) => {
          if (rsp.data === null) {
            // 审批流不存在，直接保存
            ...
            // 流程分支参数
            this.popSettingsData.sponsorDialog.form_data = { qty: 200.0000 }
