  ...
  <div class="app-container">
    <div class="edit-container">
      ...
      >
        ...
        <el-button-group>
          <el-button
            :disabled="settings.btnPoOrderDisabledStatus.disabledInsert"
            type="primary"
            icon="el-icon-circle-plus-outline"
            :loading="settings.loading"
            @click="handlePoOrderInsert"
          >添加</el-button>
          <el-button
            :disabled="settings.btnPoOrderDisabledStatus.disabledDelete"
            type="primary"
            icon="el-icon-circle-close"
            :loading="settings.loading"
            @click.native="handlePoOrderDelete"
          >删除</el-button>
        </el-button-group>
...
export default {
  ...
  components: { RadioDict, numeric, poorder_list_template, bank_list_template, BpmDialog, SupplierDialog, SimpleUploadMutilFile, previewCard },
  mixins: [],
  props: {
    data: {
      type: Object,
    ...
    handlePoOrderCurrentChange (row) {
      ...
      if (row.id) {
        getApi({ id: row.id }).then(response => {
          ...
          // 设置可申请预付款金额
          row.po_can_advance_payment_amount = _data.advance_amount_total
        }).catch(error => {
