  ...
  <div class="app-container">
    ...
    <div>
      ...
      >
        ...
        <el-form-item label="">
          <el-input
            v-model.trim="dataJson.searchForm.order_volume"
            clearable
            placeholder="订单量"
            @keyup.enter.native="handleSearch"
          />
    ...
    >
      <div class="right">
        ...
        <span class="count-title">合同总金额：</span><span class="count-data">{{ dataJson.sumData.contract_amount_total == null ? 0 : formatCurrency(dataJson.sumData.contract_amount_total) }}</span>
    ...
    >
      ...
      >
        <template v-slot="scope">
          {{ scope.row.contract_amount_sum == null ? '' : formatNumber(scope.row.contract_amount_sum, true, 4) }}
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
...
import {
  ...
  delApi,
  getApi
} from '@/api/40_business/socontract/socontract'
...
import print_template from '@/views/40_business/socontract/tabs/60_print/index.vue'
import push_template from '@/views/40_business/soorder/dialog/push/index.vue'
import cancelConfirmDialog from './dialog/cancelconfirm.vue'

export default {
  components: { cancelConfirmDialog, push_template, print_template, SimpleUpload, Pagination },
  ...
  methods: {
    ...
    handleCurrentChange (row) {
      ...
      if (this.dataJson.currentJson.id !== undefined) {
        ...
        // 推送按钮高亮（标准合同，没有采购订单）（框架合同高亮）
        if (this.dataJson.currentJson.existence_order && this.dataJson.currentJson.status === constants_dict.DICT_B_SO_CONTRACT_STATUS_TWO) {
    ...
    handleDel () {
      const _data = deepCopy(this.dataJson.currentJson)
      // 状态为待审批才可以删除
      if (_data.status.toString() !== constants_type.DICT_B_SO_CONTRACT_STATUS_ZERO) {
        this.showErrorMsg('销售合同状态异常')
        return
      }
      this.$confirm('删除后无法恢复，确认要删除该条数据吗？', '确认信息', {}).then(() => {
        this.handleDelOk()
      }).catch(action => {
        // 右上角X
        if (action !== 'close') {
    ...
    handleTabsClick (tab, event) {
      ...
      switch (tab.index) {
        case '1': // 待审批
          this.dataJson.searchForm.status = constants_type.DICT_B_SO_CONTRACT_STATUS_ZERO
          break
        case '2': // 审批中
          this.dataJson.searchForm.status = constants_type.DICT_B_SO_CONTRACT_STATUS_ONE
          break
        case '3': // 执行中
          this.dataJson.searchForm.status = constants_type.DICT_B_SO_CONTRACT_STATUS_TWO
          break
        case '4': // 已完成
          this.dataJson.searchForm.status = constants_type.DICT_B_SO_CONTRACT_STATUS_SIX
          break
        case '5': // 驳回
          this.dataJson.searchForm.status = constants_type.DICT_B_SO_CONTRACT_STATUS_THREE
          break
        case '6': // 作废审批中
          this.dataJson.searchForm.status = constants_type.DICT_B_SO_CONTRACT_STATUS_FOUR
          break
        case '7': // 已作废
          this.dataJson.searchForm.status = constants_type.DICT_B_SO_CONTRACT_STATUS_FIVE
          break
        default:
