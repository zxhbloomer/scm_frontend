<template>
  <el-dialog
    v-if="listenVisible"
    v-el-drag-dialog
    :title="settings.warehouse_group"
    :visible="visible"
  ...
  >
    ...
    >
      <el-row>
        ...
        >
          <el-transfer
            v-model="settings.transfer.group_warehouses"
            filterable
...
import { getWarehouseTransferListApi, setWarehouseTransferApi } from '@/api/40_business/warehouse/warehousegroup'
...
export default {
  ...
  watch: {
    ...
    'settings.transfer.staff_positions': {
      handler (newVal, oldVal) {
        const listA = newVal
        const listB = this.settings.transfer.old_group_warehouses
  ...
  methods: {
    ...
    // 初始化
    initEdit () {
      this.settings.warehouse_group = '维护仓库组【' + this.data.name + '】'
      // 初始化数据
      ...
      this.settings.transfer.warehouse_group_id = this.id
      getWarehouseTransferListApi(this.settings.transfer).then(response => {
        ...
        this.settings.transfer.group_warehouses = response.data.group_warehouse
        this.settings.transfer.old_group_warehouses = deepcopy(response.data.group_warehouses)
      }).finally(() => {
    ...
    // 查看仓库组数据
    initView () {
      this.settings.warehouse_group = '查看仓库组【' + this.data.name + '】'
      // 初始化数据
      this.settings.transfer = {
        warehouse_group_id: null,
        // 所有仓库
      ...
      getWarehouseTransferListApi(this.settings.transfer).then(response => {
        ...
        this.settings.transfer.group_warehouses = response.data.group_warehouse
        this.settings.transfer.group_warehouses = deepcopy(response.data.group_warehouse)
      }).finally(() => {
        this.settings.loading = false
      })
      this.settings.btnDisabledStatus.disabledReset = true
    ...
    // 插入逻辑：仓库组-仓库 维护，点击确定按钮
    doInsert () {
      // 设置仓库
      setWarehouseTransferApi(this.settings.transfer).then((_data) => {
        this.settings.loading = true
        this.settings.transfer.current_row.group_warehouse_count = _data.data.group_warehouse_count
        this.$notify({
          title: '更新处理成功',
