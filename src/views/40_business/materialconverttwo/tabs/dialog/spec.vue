...
import { getConvertGoodsListApi } from '@/api/30_wms/spec/spec'
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      this.dataJson.searchForm.pageCondition.size = this.dataJson.paging.size
      this.dataJson.searchForm.covert_type = this.type
      // 传入 owner_id
      this.dataJson.searchForm.owner_id = this.data.owner_id
      // 查询逻辑
      this.settings.loading = true
      // 物料转换 组件查询
      getConvertGoodsListApi(this.dataJson.searchForm).then(response => {
    ...
    doResetSearch () {
      ...
      this.dataJson.searchForm.owner_id = this.data
    },
