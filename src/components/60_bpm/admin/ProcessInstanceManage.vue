<template>
  <div v-loading="loading">
    ...
    <el-table :data="dataList" :header-cell-style="{background: '#e8e8e8'}" style="width: 100%; margin: 20px 0" @row-click="showProcess">
      ...
      <el-table-column show-overflow-tooltip prop="startTime" label="提交时间" min-width="120px" />
      <el-table-column show-overflow-tooltip prop="finishTime" label="结束时间" min-width="120px" />
...
export default {
  ...
  methods: {
    ...
    getDuration (row) {
      const end = this.$isNotEmpty(row.finishTime) ? row.finishTime : moment().format('YYYY-MM-DD HH:mm:ss')
