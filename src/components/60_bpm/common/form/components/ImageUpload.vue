  ...
  <div>
    ...
    <div v-else class="img-preview">
      <div class="img-preview-pc">
        <el-image v-for="(img, index) in _value" :key="index" :alt="img.name" :src="$getRes(img.url)" :preview-src-list="pcImgList" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    uploadSuccess (response, file, fileList) {
      ...
      const ft = fileList.filter(f => f.size !== undefined)
    ...
    handleRemove (file, fileList) {
      const i = this._value.findIndex(v => v.name === file.name)
      if (i > -1) {
