<template>
  <div>
    ...
    <div v-else-if="mode === 'PC' && !readonly">
      <el-input v-model="_value" :disabled="!enableEdit" size="medium" clearable :placeholder="placeholder">
        <el-button slot="append" icon="el-icon-map-location" @click="visible = true" />
      </el-input>
      <w-dialog v-model="visible" :title="'点击地图选择位置：' + position.addr" @ok="selectOk">
        <div v-if="visible" class="v-map">
          <el-amap-search-box class="search-box" :search-option="searchOption" :on-search-result="onSearchResult" />
          <el-amap class="amap-box" vid="amapDemo" :zoom="zoom" :center="center" :events="events">
            <!-- 标记点 -->
            <el-amap-marker v-if="position.zb.length > 0" vid="marker" :position="position.zb" />
          </el-amap>
        </div>
      </w-dialog>
    </div>
    <div v-else-if="mode === 'MOBILE' && !readonly" class="m-v-map">
      <field v-model="_value" :readonly="!enableEdit" clearable :placeholder="placeholder">
        <template #right-icon>
          <i class="el-icon-map-location" @click="selectPosition" />
        </template>
      </field>
  ...
  methods: {
    toLocation () {
      if (this.$isNotEmpty(this._value)) {
        var MSearch = new Map.PlaceSearch(this.searchOption) // 构造PlaceSearch类
        Map.event.addListener(MSearch, 'complete', val => {
          if (val.poiList.count > 0) {
            const center = val.poiList.pois[0]
    ...
    getPosition () {
      var geolocation = new Map.Geolocation({
        ...
        panToLocation: true,
        showCircle: true,
        buttonOffset: new Map.Pixel(10, 20),
        zoomToAccuracy: true
      })
