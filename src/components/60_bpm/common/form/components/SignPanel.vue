  ...
  <div>
    ...
    <div v-else-if="mode === 'PC' && !readonly">
      <div v-if="$isNotEmpty(_value)">
        <img :src="_value" width="40%" @click="showSignPanel">
        <div>点击签名重签</div>
      </div>
      <el-button v-else size="small" icon="el-icon-edit" @click="showSignPanel">点击签字</el-button>
      <w-dialog v-model="visible" title="请使用鼠标签字" width="700px" @ok="signOk">
        <canvas id="signPanel" width="650px" height="300px" />
      </w-dialog>
    </div>
    <div v-else-if="mode === 'MOBILE' && !readonly" class="m-sign">
      ...
      <field v-else readonly :placeholder="placeholder" @click="showSignPanel">
        <template #right-icon>
      ...
      <popup v-model="visible" :style="popupStyle" position="left" lazy-render safe-area-inset-bottom>
        ...
        <div class="m-sign-panel">
          <canvas id="signPanel" height="200px" />
        </div>
      </popup>
