<template>
  <el-table v-if="selectedNode.props.operationPerm" :header-cell-style="{background:'#f5f6f6'}" :data="operatePerms" border style="width: 100%">
    <el-table-column prop="name" show-overflow-tooltip label="操作项" />
    <el-table-column prop="alisa" show-overflow-tooltip label="自定义别名">
      <template slot-scope="scope">
        <el-input v-model="selectedNode.props.operationPerm[scope.row.symbol].alisa" size="small" clearable />
      </template>
    </el-table-column>
    <el-table-column prop="show" show-overflow-tooltip label="是否启用">
      <template slot-scope="scope">
        <el-switch v-model="selectedNode.props.operationPerm[scope.row.symbol].show" :disabled="disable(scope.row.symbol)" />
      </template>
    </el-table-column>
  </el-table>
</template>
